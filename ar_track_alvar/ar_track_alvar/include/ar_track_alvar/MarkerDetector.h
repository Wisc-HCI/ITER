...
protected:
	...
	virtual void _blockCorners_append(unsigned long id, std::vector<PointDouble> corners) = 0;
	...
	std::map<unsigned long, std::vector<PointDouble>>* blobCorners;
	double edge_length;
	int res;
	double margin;
...
public:
	...
	*/
	...
	std::map<unsigned long, std::vector<PointDouble>>& GetBlobCorners() { return *this->blobCorners; }
};
...
protected:
  ...
  void _markers_clear() { markers->clear(); blobCorners->clear(); }
  void _markers_push_back(Marker *mn) { markers->push_back(*((M*)mn));}
  size_t _markers_size() { return markers->size(); }
	void _blockCorners_append(unsigned long id, std::vector<PointDouble> corners) { blobCorners->insert({id,corners}); }
...
public:
  ...
  MarkerDetector() : MarkerDetectorImpl() {
    ...
    track_markers = new std::vector<M, Eigen::aligned_allocator<M> >;
		blobCorners = new std::map<unsigned long, std::vector<PointDouble>>;
	}
